...
function CPColorWithImages()
{
    if (arguments.length < 3)
    {
        ...
        for (var i = 0; i < slices.length; ++i)
        {
            ...
            imageSlices.push(slice ? RTImageInBundle(slice[0], CGSizeMake(slice[1], slice[2]), slice[3]) : nil);
        }
    ...
    {
        return [CPColor colorWithPatternImage:RTImageInBundle(arguments[0], CGSizeMake(arguments[1], arguments[2]), arguments[3])];
    }
    else
    {
        return nil;
...
*/
+ (CPColor)colorWithPatternImage:(RTImage)anImage
{
...
/* @ignore */
- (id)_initWithPatternImage:(RTImage)anImage
{
    self = [super init];
