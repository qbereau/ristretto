...
+ (void)initialize
{
    ...
    if (!systemFontFace)
        systemFontFace = [[CPBundle bundleForClass:[RTView class]] objectForInfoDictionaryKey:@"CPSystemFontFace"];
    ...
    if (!systemFontSize)
        systemFontSize = [[CPBundle bundleForClass:[RTView class]] objectForInfoDictionaryKey:@"CPSystemFontSize"];
...
{
    return _CPFonts[_CPFontCreateCSSString(_CPFontNormalizedNameArray(aName).join(", "), aSize, bold, italic)] || [[CPFont alloc] _initWithName:aName size:aSize bold:bold italic:italic];
}
